...
import {
  ...
  effectiveStatus,
} from '@/services/teamInvitations.service'
...
export function TeamInvitePage() {
  ...
  const invitation = useAsync(() => getTeamInvitation(code), [code])
  ...
  async function respond(accept: boolean) {
    ...
    try {
      if (accept) {
        await acceptTeamInvitation(inv, profile)
        setDone('accepted')
        toast.success(`You've joined ${inv.teamName}`)
      } else {
        await declineTeamInvitation(inv, profile)
        setDone('declined')
      }
    } catch (e) {
  ...
  return (
    <div className="mx-auto max-w-md py-12">
      <Card className="p-6 text-center">
        <Shield size={36} className="mx-auto text-brand-600" />
        <h1 className="mt-3 text-xl font-bold text-ink-900 dark:text-ink-50">
          Invitation to join {inv.teamName}
        </h1>
        <p className="mt-2 text-sm text-ink-500 dark:text-ink-400">
          {inv.createdByName} invited @{inv.invitedUsername} to join {inv.teamName}'s roster.
        </p>
        {inv.message && (
        ...
        {status === 'accepted' && (
          <p className="mt-5 flex items-center justify-center gap-2 font-semibold text-pitch-700">
            <Check size={18} /> Invitation accepted — you're on the roster.
          </p>
        )}
        {status === 'declined' && (
