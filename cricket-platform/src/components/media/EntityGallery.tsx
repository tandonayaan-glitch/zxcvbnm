...
/**
 * Generic Storage-backed photo gallery. Read-only for visitors without `canManage`; owners get an
 * upload control and a per-photo delete button. Extracted from the match-only `MatchGallery` so
 * any entity can get a gallery from its own Storage folder without duplicating this logic.
 */
export function EntityGallery({
  ...
  canManage: boolean
  emptyLabel?: string
  /** When true (the default — matches `MatchGallery`'s original behavior), a read-only visitor
   *  sees nothing at all for an empty gallery rather than an empty card. Set false for a gallery
   *  living inside its own dedicated tab/section, where "No photos yet." is the expected,
   *  unsurprising result of clicking in — same convention as the Activity tab's empty state. */
  ...
  if (hideWhenEmpty && !canManage && !images.loading && photos.length === 0) return null
      ...
      <CardHeader
        title={
          <span className="flex items-center gap-2">
            <ImageIcon size={18} className="text-ink-500" /> {title}
          </span>
        }
      ...
      <CardBody>
        ...
        {images.loading ? (
          ...
          <p className="py-4 text-center text-sm text-ink-500 dark:text-ink-400">
            {canManage ? 'No photos yet — add the first one.' : emptyLabel}
          </p>
        ) : (
