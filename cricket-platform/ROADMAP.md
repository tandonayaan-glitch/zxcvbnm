...
- **Still auth-blocked (no test credentials — account creation & password sign-in are disallowed by policy):** every signed-in surface (real match creation, a full scoring session, admin panels, the User-A-vs-User-B audit-log check). Items 3/6/8 are verified by code trace + route-guard + rules review + pure-logic runtime tests against real modules. `tsc` 0 / `lint` 0 errors / `build` green. **Not deployed.**
